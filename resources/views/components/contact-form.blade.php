@props(['services'])
<div class="reveal rounded-xl border border-slate-200 bg-white p-7 shadow-sm shadow-slate-900/[.03] sm:p-10" data-reveal>
    ...
    <form action="{{ route('contact.store') }}" method="POST" class="grid gap-5 sm:grid-cols-2">@csrf
        ...
        <div class="sm:col-span-2"><x-button-primary type="submit">Envoyer la demande</x-button-primary><p class="mt-4 text-xs leading-5 text-slate-400">En envoyant ce formulaire, vous acceptez notre <a class="underline hover:text-coral" href="{{ route('confidentialite') }}">politique de confidentialité</a>.</p></div>
    </form>
</div>
